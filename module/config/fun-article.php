...
if(get_boxmoe('blog_border') == 'border2'){$border='blog-card';}if(get_boxmoe('blog_border') == 'border3'){$border='';}
return  $border;	
}
//wow
...
function lightbox_comment ($content) {
	...
	$replacement = '<a$1href=$2$3$4$5$6 data-fancybox="images" data-fancybox-group="button"><img$1src=$2$3$4$5$6 alt="评论图片">';
	$content = preg_replace($pattern, $replacement, $content);
	return $content;
}
// fancybox-erphpdown
...
function boxmoe_posts_related($title='', $limit=6, $model='thumb'){
    ...
    if ( $posttags ) { 
        ...
        while( have_posts() ) { the_post();
            echo '<div class="col-lg-4"><div class="card card-plain card-blog">'; 

            if( $model == 'thumb' ){
             echo '<div class="card-image border-radius-lg position-relative cursor-pointer"><div class="blur-shadow-image"><a'._post_target_blank().' href="'.get_permalink().'"><img class="img img-raised" '._get_post_thumbnail(). '></a></div></div>';
            }
            echo '<div class="card-body px-0">';
			echo'<p>'.mb_strimwidth(get_the_title(), 0, 30, '…').'</p>';
			echo'<a '._post_target_blank().' href="'.get_permalink().'" title="'.get_the_title().'"class="text-info icon-move-right">阅读更多<i class="fa fa-arrow-right text-sm ms-1"></i></a>';
			echo'</div></div></div>'; 
...
function boxmoe_paging() {
   ...
   if( get_boxmoe('paging_type') == 'multi' && $paged !== 1 ) p_link(0);
    ...
    if( get_boxmoe('paging_type') == 'next' ){echo '<li class="page-item">'; previous_posts_link(__('« 上一页', 'boxmoe-com')); echo '</li>';}
   ...
   // echo '<li class="page-item">'; next_posts_link(__('Next', 'boxmoe-com')); echo '</li>';
    ...
    if( get_boxmoe('paging_type') == 'next' ){echo '<li class="page-item">'; next_posts_link(__('下一页 »', 'boxmoe-com')); echo '</li>';}
