...
<?php if ( $args['type'] != 'pings' ) : ?>
                                                      ...
                                                      <div class="comment-body">
                                                            ...
                                                            <?php if($comment->user_id > 1){
																echo '<span class="commentsbadge members"><i class="fa fa-heart"></i> '.get_boxmoe('comnanesu').'</span>'; 
																}if($comment->user_id == 1){
																echo '<span class="commentsbadge blogger"><i class="fa fa-check-square"></i> '.get_boxmoe('comnanes').'</span>'; 
																}if($comment->user_id == 0){
																echo '<span class="commentsbadge tourists"><i class="fa fa-globe"></i> '.get_boxmoe('comnaness').'</span>'; }
																?>
...
function boxmoe_comment_add_at( $comment_text, $comment = '') {  
  if( $comment->comment_parent > 0 & $comment->user_id == 1) {  
    $comment_text = '<span class="commentsbadge badge-pill atwho">@ '.get_comment_author( $comment->comment_parent ) . '</span> ' . $comment_text.'';  
  }  
  if( $comment->comment_parent > 0 & $comment->user_id != 1) {  
    $comment_text = '<span class="commentsbadge badge-pill atwho">@ '.get_comment_author( $comment->comment_parent ) . '</span> ' . $comment_text;  
  }
...
function comment_mail_notify($comment_id) {
  ...
  if (($parent_id != '') && ($spam_confirmed != 'spam')) {
    ...
    $message = '
    <table style="width:60%;margin: 0 auto;height:100% "><tbody><tr><td style="background:#fafafa url()">
