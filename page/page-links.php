                            ...
                            <div class="single-content">
                                ...
                                if (!empty($categories) && !is_wp_error($categories)) {
                                    foreach ($categories as $category) {
                                        ...
                                        echo '<div class="thw-sept"></div><div class="link-title mt-4 mb-8" >' . esc_html($category->name) . '</div>';
                                        echo '<div class="row">';
                                ...
                                foreach ($all_links as $link) {
                                    $categories = get_the_terms($link->link_id, 'link_category');
                                    if (empty($link->link_description)){
                                        $desc ='这站长太懒了什么也没留下';
                                    }else{
                                        $desc = $link->link_description;
                                    ...
                                    if (empty($link->link_image)){
                                        $webimg = $webimg = boxmoe_themes_dir().'/assets/images/linkspic.jpg';
                                    }else{
                                        $webimg = $link->link_image;
                                    }
                                    if (empty($categories) || is_wp_error($categories)) {
